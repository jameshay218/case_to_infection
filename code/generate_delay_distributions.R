...
range(kudos_dat_china$delay,na.rm=TRUE)
...
number_confirmed <- kudos_dat_china %>% filter(delay > 0) %>% group_by(reporting_date) %>% tally()
...
for(i in seq_along(use_dates)) {
  ...
  tmp <- kudos_dat_china %>% filter(delay > 0 & reporting_date <= start_date &
                                   reporting_date > end_date ) %>% count(delay) %>% mutate(delay=as.numeric(delay))
  tmp <- tmp %>% mutate(start=use_dates[i])
  x <- kudos_dat_china %>% filter(delay > 0 & reporting_date <= start_date &
...
number_onsets <- kudos_dat_china %>% filter(delay > 0) %>% group_by(symptom_onset) %>% tally()
...
for(i in seq_along(use_dates)) {
  ...
  ## Get the data for this period
  tmp <- kudos_dat_china %>% filter(delay > 0 & symptom_onset <= start_date &
                                symptom_onset > end_date ) %>% count(delay) %>% mutate(delay=as.numeric(delay))
  tmp <- tmp %>% mutate(start=use_dates[i])
  x <- kudos_dat_china %>% filter(delay > 0 & symptom_onset <= start_date &
