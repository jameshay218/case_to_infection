...
for(i in seq_along(use_dates)) {
  ...
  tmp <- combined_dat %>% filter(confirmation_delay > 0 & date_confirmation <= start_date &
                                   date_confirmation > end_date ) %>% count(confirmation_delay) %>% mutate(confirmation_delay=as.numeric(confirmation_delay))
  tmp <- tmp %>% mutate(start=use_dates[i])
  x <- combined_dat %>% filter(confirmation_delay > 0 & date_confirmation <= start_date &
                              date_confirmation > end_date) %>% pull(confirmation_delay) %>% as.numeric
  ## VERY IMPORTANT
  ## We assume that it takes at least a day to get confirmed (ie. the ceiling of whatever
  ## randomly sampled confirmation confirmation_delay time you got) 
...
colnames(model_probs_geometric_backward) <- c("label", "confirmation_delay","prob")
...
p_sliding_delays_backward <- ggplot(all_dat_backward) + 
  ...
  geom_line(data=all_model_probs_backward,aes(x=confirmation_delay,y=prob, col=fit),size=1) +
...
png("plots/delay_distribution_backward_mortiz.png",height=6,width=8,units="in",res=300)
...
number_onsets <- combined_dat %>% filter(confirmation_delay > 0) %>% group_by(date_onset_symptoms) %>% tally()
...
## is greater than threshold
first_date <- number_onsets$date_onset_symptoms[which(cumsum(number_onsets$n) > threshold)[1]]
use_dates <- dates[dates > first_date]
probs <- numeric(length(use_dates))
...
for(i in seq_along(use_dates)) {
  ...
  ## Go back in time and accumlate cases until >threshold cases
  while(counted < threshold & end_date > min(dates)){
    tmp_count <- number_onsets %>% filter(date_onset_symptoms == end_date) %>% pull(n)
    counted <- counted + max(tmp_count,0)
    end_date <- end_date - 1
  }
  
  ## Get the data for this period
  tmp <- combined_dat %>% filter(confirmation_delay > 0 & date_onset_symptoms <= start_date &
                                date_onset_symptoms > end_date ) %>% count(confirmation_delay) %>% mutate(confirmation_delay=as.numeric(confirmation_delay))
  tmp <- tmp %>% mutate(start=use_dates[i])
  x <- combined_dat %>% filter(confirmation_delay > 0 & date_onset_symptoms <= start_date &
                              date_onset_symptoms > end_date) %>% pull(confirmation_delay) %>% as.numeric
  ## VERY IMPORTANT
  ## We assume that it takes at least a day to get confirmed (ie. the ceiling of whatever
...
colnames(model_probs_geometric_forward) <- c("label", "confirmation_delay","prob")
...
p_sliding_delays_forward <- ggplot(all_dat_forward) + 
  geom_bar(aes(x=confirmation_delay,y=rel_n),stat="identity") + 
  geom_line(data=all_model_probs_forward,aes(x=confirmation_delay,y=prob, col=fit),size=1) +
  geom_vline(xintercept=1,linetype="dashed") +
  coord_cartesian(xlim=c(0,40)) +
  facet_wrap(~label) +
  theme_bw() +
  ...
  theme(legend.position=c(0.9,0.1))
png("plots/delay_distribution_forward_mortiz.png",height=6,width=8,units="in",res=300)
