...
sim_data_quantiles_province <- province_data1 %>% ungroup() %>% group_by(x, province) %>% 
  do(data.frame(t(quantile(.$n, probs = c(0.025,0.5,0.975),na.rm=TRUE)))) %>% ungroup()
...
max_help <- dat %>%
  group_by(province) %>%
  summarise(max_truncate=max(upper)+5) %>% right_join(dat) %>%
  ...
  mutate(median_h=ifelse(median_h >  max_truncate,  NA, median_h)) %>% ungroup()
